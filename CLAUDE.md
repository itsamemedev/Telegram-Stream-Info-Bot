...
`bot.py` hat **29.437 Zeilen / 1,5 MB ≈ 385.000 Token**. Diese Datei wird
...
`find` antwortet aus `.claude/INDEX.md` — 359 Routen (161 in `bot.py`, 198 in
`nc/routes/`), 45 Slash-Commands, 515 Funktionen mit Zeilennummern. Nach Änderungen an Routen, Commands oder
...
## Aufbau
    ...
    bot.py               Monolith: Telegram + Discord (45 Slash-Commands),
                         Flask-Dashboard (161 eigene Routen), Scraper, Recorder,
    ...
    nc/                  93 Fachmodule: db, scraping, restream, oauth, ledger,
                         i18n, …
    nc/routes/           20 Flask-Blueprints mit 198 weiteren API-Routen
