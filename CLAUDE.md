...
`bot.py` hat **28.133 Zeilen / 1,4 MB ≈ 356.000 Token**. Diese Datei wird
...
`find` antwortet aus `.claude/INDEX.md` — 359 Routen (105 in `bot.py`, 254 in
`nc/routes/`), 45 Slash-Commands, 491 Funktionen mit Zeilennummern. Nach Änderungen an Routen, Commands oder
...
## Aufbau
    ...
    bot.py               Monolith: Telegram + Discord (45 Slash-Commands),
                         Flask-Dashboard (105 eigene Routen), Scraper, Recorder,
    ...
    nc/                  102 Fachmodule: db, scraping, restream, oauth, ledger,
                         i18n, …
    nc/routes/           29 Flask-Blueprints mit 254 weiteren API-Routen
