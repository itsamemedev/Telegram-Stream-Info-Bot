...
flowchart TB
    ...
    BOT["bot.py<br/>Monolith · 24.296 Zeilen<br/>Scraper · Recorder · Restream<br/>Flask-Dashboard · 34 eigene Routen"]:::core

    BOT -->|configure| NC["nc/ — 143 Fachmodule<br/>Schema · OAuth · Restream<br/>Ledger · Moderation · Intel"]:::lib
...
Alle Einstellungen leben in `.env`. Die Vorlage `.env.example` kennt **rund 529 Variablen** — das Minimum ist klein:
...
`bot.py` hat 24.296 Zeilen. Es wird **nie** ganz gelesen und **nie**
...
(34 in `bot.py`, 330 in `nc/routes/`), 60 Slash-Commands, 546 Funktionen,
...
| Regel | Warum |
|---|---|
| **`.env` liegt nie im Repo und nie im Archiv** | ~529 Variablen inkl. Cookies, OAuth-Tokens und Stream-Keys |
...
| Fachmodule | 143 in `nc/` (+36 in `nc/routes/`, +3 in `nc/intel/`), 10 in `brain/` |
| Sentinel-Agenten | 13 |
...
24.296 Zeilen**. Die Datei ist der Engpass des Projekts. Die Messlatte dafür ist
