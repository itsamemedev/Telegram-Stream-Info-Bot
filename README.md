...
### 📊 Auswerten
Flask-Dashboard mit **359 API-Routen**, Wissensgraph-Visualisierung,
...
| **Dashboard** | 359 Flask-Routen · Live-Panels · Gehirn-Visualisierung mit Lernkurve · Overlay für OBS · installierbare PWA (Android) · QR-Login |
...
flowchart TB
    ...
    BOT["bot.py<br/>Monolith · 28.133 Zeilen<br/>Scraper · Recorder · Restream<br/>Flask-Dashboard · 105 eigene Routen"]:::core

    BOT -->|configure| NC["nc/ — 102 Fachmodule<br/>Schema · OAuth · Restream<br/>Ledger · Moderation · Intel"]:::lib
    NC --> RT["nc/routes/ — 29 Blueprints<br/>254 weitere API-Routen"]:::lib
...
Alle Einstellungen leben in `.env`. Die Vorlage `.env.example` kennt **rund 497 Variablen** — das Minimum ist klein:
...
Flask-Dashboard mit **359 Routen** unter `127.0.0.1:8050`.
...
`bot.py` hat 28.133 Zeilen. Es wird **nie** ganz gelesen und **nie**
...
(105 in `bot.py`, 254 in `nc/routes/`), 45 Slash-Commands, 491 Funktionen,
...
| Regel | Warum |
|---|---|
| **`.env` liegt nie im Repo und nie im Archiv** | ~497 Variablen inkl. Cookies, OAuth-Tokens und Stream-Keys |
...
│
├── nc/                       102 Fachmodule (bot-frei, configure()-Injection)
...
| Flask-Routen | 359 (105 in `bot.py` · 254 in `nc/routes/`) |
| Discord-Slash-Commands | 45 |
| Fachmodule | 102 in `nc/` (+29 in `nc/routes/`, +3 in `nc/intel/`), 10 in `brain/` |
| Sentinel-Agenten | 13 |
| Konfigurationsvariablen | ~497 |
...
28.133 Zeilen**. Die Datei ist der Engpass des Projekts. Die Messlatte dafür ist
...
`nc/routes/` trägt heute 29 Blueprints mit 254 API-Routen, die nicht mehr im
Monolithen stehen.
