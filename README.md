...
### 📊 Auswerten
Flask-Dashboard mit **360 API-Routen**, Wissensgraph-Visualisierung,
...
| **Dashboard** | 360 Flask-Routen · Live-Panels · Gehirn-Visualisierung mit Lernkurve · Overlay für OBS · installierbare PWA (Android) · QR-Login |
...
flowchart TB
    ...
    BOT["bot.py<br/>Monolith · 25.723 Zeilen<br/>Scraper · Recorder · Restream<br/>Flask-Dashboard · 34 eigene Routen"]:::core

    BOT -->|configure| NC["nc/ — 121 Fachmodule<br/>Schema · OAuth · Restream<br/>Ledger · Moderation · Intel"]:::lib
    NC --> RT["nc/routes/ — 36 Blueprints<br/>326 weitere API-Routen"]:::lib
...
Alle Einstellungen leben in `.env`. Die Vorlage `.env.example` kennt **rund 500 Variablen** — das Minimum ist klein:
...
Flask-Dashboard mit **360 Routen** unter `127.0.0.1:8050`.
...
`bot.py` hat 25.723 Zeilen. Es wird **nie** ganz gelesen und **nie**
...
`find` antwortet aus **[`.claude/INDEX.md`](.claude/INDEX.md)** — 360 Routen
(34 in `bot.py`, 326 in `nc/routes/`), 45 Slash-Commands, 468 Funktionen,
...
| Regel | Warum |
|---|---|
| **`.env` liegt nie im Repo und nie im Archiv** | ~500 Variablen inkl. Cookies, OAuth-Tokens und Stream-Keys |
...
│
├── nc/                       118 Fachmodule (bot-frei, configure()-Injection)
...
| Flask-Routen | 360 (34 in `bot.py` · 326 in `nc/routes/`) |
| Discord-Slash-Commands | 45 |
| Fachmodule | 121 in `nc/` (+36 in `nc/routes/`, +3 in `nc/intel/`), 10 in `brain/` |
| Sentinel-Agenten | 13 |
...
25.723 Zeilen**. Die Datei ist der Engpass des Projekts. Die Messlatte dafür ist
...
`nc/routes/` trägt heute 36 Blueprints mit 326 API-Routen, die nicht mehr im
Monolithen stehen.
