...
flowchart TB
    ...
    BOT["bot.py<br/>Monolith · 28.467 Zeilen<br/>Scraper · Recorder · Restream<br/>Flask-Dashboard · 111 eigene Routen"]:::core

    BOT -->|configure| NC["nc/ — 101 Fachmodule<br/>Schema · OAuth · Restream<br/>Ledger · Moderation · Intel"]:::lib
    NC --> RT["nc/routes/ — 28 Blueprints<br/>248 weitere API-Routen"]:::lib
...
`bot.py` hat 28.467 Zeilen. Es wird **nie** ganz gelesen und **nie**
...
(111 in `bot.py`, 248 in `nc/routes/`), 45 Slash-Commands, 494 Funktionen,
...
│
├── nc/                       101 Fachmodule (bot-frei, configure()-Injection)
...
| Flask-Routen | 359 (111 in `bot.py` · 248 in `nc/routes/`) |
| Discord-Slash-Commands | 45 |
| Fachmodule | 101 in `nc/` (+28 in `nc/routes/`, +3 in `nc/intel/`), 10 in `brain/` |
| Sentinel-Agenten | 13 |
...
28.467 Zeilen**. Die Datei ist der Engpass des Projekts. Die Messlatte dafür ist
...
`nc/routes/` trägt heute 28 Blueprints mit 248 API-Routen, die nicht mehr im
Monolithen stehen.
