...
<img src="docs/assets/banner.svg" alt="NIGHTCRAWLER — v4.3.1 Freie Sicht" width="100%">
...
flowchart TB
    ...
    BOT["bot.py<br/>Monolith · 24.148 Zeilen<br/>Scraper · Recorder · Restream<br/>Flask-Dashboard · 34 eigene Routen"]:::core

    BOT -->|configure| NC["nc/ — 141 Fachmodule<br/>Schema · OAuth · Restream<br/>Ledger · Moderation · Intel"]:::lib
...
Alle Einstellungen leben in `.env`. Die Vorlage `.env.example` kennt **rund 528 Variablen** — das Minimum ist klein:
...
| `/cookies` · `/cookies holen [browser]` · `/teststream` | Cookie-Zustand, Cookies automatisch beziehen, Teststream |
...
`bot.py` hat 24.148 Zeilen. Es wird **nie** ganz gelesen und **nie**
...
(34 in `bot.py`, 330 in `nc/routes/`), 60 Slash-Commands, 544 Funktionen,
...
| Regel | Warum |
|---|---|
| **`.env` liegt nie im Repo und nie im Archiv** | ~528 Variablen inkl. Cookies, OAuth-Tokens und Stream-Keys |
...
| Fachmodule | 141 in `nc/` (+36 in `nc/routes/`, +3 in `nc/intel/`), 10 in `brain/` |
| Sentinel-Agenten | 13 |
| Konfigurationsvariablen | ~528 |
...
24.148 Zeilen**. Die Datei ist der Engpass des Projekts. Die Messlatte dafür ist
...
**NIGHTCRAWLER v4.3.1 · „Freie Sicht"**
