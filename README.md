...
flowchart TB
    ...
    BOT["bot.py<br/>Monolith · 27.624 Zeilen<br/>Scraper · Recorder · Restream<br/>Flask-Dashboard · 89 eigene Routen"]:::core

    BOT -->|configure| NC["nc/ — 104 Fachmodule<br/>Schema · OAuth · Restream<br/>Ledger · Moderation · Intel"]:::lib
    NC --> RT["nc/routes/ — 30 Blueprints<br/>270 weitere API-Routen"]:::lib
...
Alle Einstellungen leben in `.env`. Die Vorlage `.env.example` kennt **rund 499 Variablen** — das Minimum ist klein:
...
`bot.py` hat 27.624 Zeilen. Es wird **nie** ganz gelesen und **nie**
...
(89 in `bot.py`, 270 in `nc/routes/`), 45 Slash-Commands, 483 Funktionen,
...
| Regel | Warum |
|---|---|
| **`.env` liegt nie im Repo und nie im Archiv** | ~499 Variablen inkl. Cookies, OAuth-Tokens und Stream-Keys |
...
│
├── nc/                       104 Fachmodule (bot-frei, configure()-Injection)
...
| Flask-Routen | 359 (89 in `bot.py` · 270 in `nc/routes/`) |
| Discord-Slash-Commands | 45 |
| Fachmodule | 104 in `nc/` (+30 in `nc/routes/`, +3 in `nc/intel/`), 10 in `brain/` |
| Sentinel-Agenten | 13 |
| Konfigurationsvariablen | ~499 |
...
27.624 Zeilen**. Die Datei ist der Engpass des Projekts. Die Messlatte dafür ist
...
`nc/routes/` trägt heute 30 Blueprints mit 270 API-Routen, die nicht mehr im
Monolithen stehen.
