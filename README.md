...
flowchart TB
    ...
    BOT["bot.py<br/>Monolith · 29.442 Zeilen<br/>Scraper · Recorder · Restream<br/>Flask-Dashboard · 153 eigene Routen"]:::core

    BOT -->|configure| NC["nc/ — 94 Fachmodule<br/>Schema · OAuth · Restream<br/>Ledger · Moderation · Intel"]:::lib
    NC --> RT["nc/routes/ — 21 Blueprints<br/>206 weitere API-Routen"]:::lib
...
Alle Einstellungen leben in `.env`. Die Vorlage `.env.example` kennt **rund 496 Variablen** — das Minimum ist klein:
...
`bot.py` hat 29.442 Zeilen. Es wird **nie** ganz gelesen und **nie**
...
(153 in `bot.py`, 206 in `nc/routes/`), 45 Slash-Commands, 510 Funktionen,
...
| Regel | Warum |
|---|---|
| **`.env` liegt nie im Repo und nie im Archiv** | ~496 Variablen inkl. Cookies, OAuth-Tokens und Stream-Keys |
...
| Flask-Routen | 359 (153 in `bot.py` · 206 in `nc/routes/`) |
| Discord-Slash-Commands | 45 |
| Fachmodule | 94 in `nc/` (+21 in `nc/routes/`, +3 in `nc/intel/`), 10 in `brain/` |
| Sentinel-Agenten | 13 |
...
29.442 Zeilen**. Die Datei ist der Engpass des Projekts. Die Messlatte dafür ist
...
`nc/routes/` trägt heute 21 Blueprints mit 206 API-Routen, die nicht mehr im
Monolithen stehen.
