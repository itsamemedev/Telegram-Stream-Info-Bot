...
flowchart TB
    ...
    BOT["bot.py<br/>Monolith · 31.689 Zeilen<br/>Scraper · Recorder · Restream<br/>Flask-Dashboard · 226 eigene Routen"]:::core

    BOT -->|configure| NC["nc/ — 90 Fachmodule<br/>Schema · OAuth · Restream<br/>Ledger · Moderation · Intel"]:::lib
    NC --> RT["nc/routes/ — 11 Blueprints<br/>129 weitere API-Routen"]:::lib
...
`bot.py` hat über 31.000 Zeilen. Es wird **nie** ganz gelesen und **nie**
...
(226 in `bot.py`, 129 in `nc/routes/`), 45 Slash-Commands, 560 Funktionen,
...
| Flask-Routen | 355 (226 in `bot.py` · 129 in `nc/routes/`) |
| Discord-Slash-Commands | 45 |
| Fachmodule | 90 in `nc/` (+11 in `nc/routes/`, +3 in `nc/intel/`), 10 in `brain/` |
| Sentinel-Agenten | 13 |
...
31.689 Zeilen**. Die Datei ist der Engpass des Projekts. Die Messlatte dafür ist
...
`nc/routes/` trägt heute 11 Blueprints mit 129 API-Routen, die nicht mehr im
Monolithen stehen.
