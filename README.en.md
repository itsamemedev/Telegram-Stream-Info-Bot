...
flowchart TB
    ...
    BOT["bot.py<br/>monolith · 29,306 lines<br/>scraper · recorder · restream<br/>Flask dashboard · 149 own routes"]:::core

    BOT -->|configure| NC["nc/ — 92 domain modules<br/>schema · OAuth · restream<br/>ledger · moderation · intel"]:::lib
    NC --> RT["nc/routes/ — 23 blueprints<br/>210 further API routes"]:::lib
...
`bot.py` has 29,306 lines. It is **never** read in full and **never** searched
...
(149 in `bot.py`, 210 in `nc/routes/`), 45 slash commands, 510 functions, each
with a line number.
...
│   ├── routes/               23 Flask blueprints (210 API routes)
...
| Flask routes | 359 (149 in `bot.py` · 210 in `nc/routes/`) |
...
The next big step is not a feature, it is cleaning up: **`bot.py` has 29,306
...
running: `nc/routes/` carries 23 blueprints with 210 API routes today that no
