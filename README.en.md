...
`bot.py` has 26,346 lines. It is **never** read in full and **never** searched
...
(37 in `bot.py`, 323 in `nc/routes/`), 45 slash commands, 476 functions, each
with a line number.
...
│   ├── routes/               35 Flask blueprints (323 API routes)
...
| Flask routes | 360 (37 in `bot.py` · 323 in `nc/routes/`) |
...
The next big step is not a feature, it is cleaning up: **`bot.py` has 26,346
...
running: `nc/routes/` carries 35 blueprints with 323 API routes today that no
