...
flowchart TB
    ...
    BOT["bot.py<br/>monolith · 27,624 lines<br/>scraper · recorder · restream<br/>Flask dashboard · 89 own routes"]:::core

    BOT -->|configure| NC["nc/ — 92 domain modules<br/>schema · OAuth · restream<br/>ledger · moderation · intel"]:::lib
    NC --> RT["nc/routes/ — 30 blueprints<br/>270 further API routes"]:::lib
...
499 variables** — the minimum is small:
...
`bot.py` has 27,624 lines. It is **never** read in full and **never** searched
...
(89 in `bot.py`, 270 in `nc/routes/`), 45 slash commands, 483 functions, each
with a line number.
...
| Rule | Why |
|---|---|
| **The `.env` is never in the repository and never in the archive** | ~499 variables including cookies, OAuth tokens and stream keys |
...
│   ├── routes/               30 Flask blueprints (270 API routes)
...
| Flask routes | 359 (89 in `bot.py` · 270 in `nc/routes/`) |
| Discord slash commands | 45 |
| Domain modules | 92 in `nc/` (+18 in `nc/routes/`, +3 in `nc/intel/`), 10 in `brain/` |
| Sentinel agents | 13 |
| Configuration variables | ~499 |
...
The next big step is not a feature, it is cleaning up: **`bot.py` has 27,624
...
running: `nc/routes/` carries 30 blueprints with 270 API routes today that no
