...
flowchart TB
    ...
    BOT["bot.py<br/>monolith · 28,133 lines<br/>scraper · recorder · restream<br/>Flask dashboard · 105 own routes"]:::core

    BOT -->|configure| NC["nc/ — 92 domain modules<br/>schema · OAuth · restream<br/>ledger · moderation · intel"]:::lib
    NC --> RT["nc/routes/ — 29 blueprints<br/>254 further API routes"]:::lib
...
`bot.py` has 28,133 lines. It is **never** read in full and **never** searched
...
(105 in `bot.py`, 254 in `nc/routes/`), 45 slash commands, 491 functions, each
with a line number.
...
│   ├── routes/               29 Flask blueprints (254 API routes)
...
| Flask routes | 359 (105 in `bot.py` · 254 in `nc/routes/`) |
...
The next big step is not a feature, it is cleaning up: **`bot.py` has 28,133
...
running: `nc/routes/` carries 29 blueprints with 254 API routes today that no
