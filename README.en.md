...
flowchart TB
    ...
    BOT["bot.py<br/>monolith · 29,254 lines<br/>scraper · recorder · restream<br/>Flask dashboard · 153 own routes"]:::core

    BOT -->|configure| NC["nc/ — 92 domain modules<br/>schema · OAuth · restream<br/>ledger · moderation · intel"]:::lib
    NC --> RT["nc/routes/ — 21 blueprints<br/>206 further API routes"]:::lib
...
`bot.py` has 29,254 lines. It is **never** read in full and **never** searched
...
(153 in `bot.py`, 206 in `nc/routes/`), 45 slash commands, 510 functions, each
with a line number.
...
│   ├── routes/               21 Flask blueprints (206 API routes)
...
| Flask routes | 359 (153 in `bot.py` · 206 in `nc/routes/`) |
...
The next big step is not a feature, it is cleaning up: **`bot.py` has 29,254
...
running: `nc/routes/` carries 21 blueprints with 206 API routes today that no
