...
`bot.py` has **29,437 lines / 1.5 MB ≈ 385,000 tokens**. That file is **never**
...
`find` answers from `.claude/INDEX.md` — 359 routes (161 in `bot.py`, 198 in
`nc/routes/`), 45 slash commands, 515 functions with line numbers. After changes
...
## Layout
    ...
    bot.py               monolith: Telegram + Discord (45 slash commands),
                         Flask dashboard (161 own routes), scraper, recorder,
    ...
    nc/routes/           20 Flask blueprints with 198 further API routes
