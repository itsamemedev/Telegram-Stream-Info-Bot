...
`bot.py` has **29,714 lines / 1.5 MB ≈ 385,000 tokens**. That file is **never**
...
`find` answers from `.claude/INDEX.md` — 359 routes (172 in `bot.py`, 187 in
`nc/routes/`), 45 slash commands, 520 functions with line numbers. After changes
...
## Layout
    ...
    bot.py               monolith: Telegram + Discord (45 slash commands),
                         Flask dashboard (172 own routes), scraper, recorder,
    ...
    nc/routes/           18 Flask blueprints with 187 further API routes
